...
class Earth3d extends Group {
    ...
    constructor(camera) {
        ...
        const innerGeometry = new SphereGeometry(atmosphere.innerRadius, 500, 500);
        const outerGeometry = new SphereGeometry(atmosphere.outerRadius, 500, 500);
        ...
        this.add(this.ground, this.sky);

        this.camera = camera;
        this.parentObj = new Group();
        this.sunvect = new Vector3(1, 0, 0);
        this.omega = 0.00007292115;

        this._tempCameraPos = new Vector3();
        this._tempSunVect = new Vector3();
        this._tempQuaternion = new Quaternion();
        this._axisY = new Vector3(0, 1, 0);

        this._uniforms = this.ground.material.uniforms;
        
        this._updateCameraPosition();
    }

    _updateCameraPosition() {
        this._tempCameraPos.copy(this.camera.position).sub(this.parentObj.position);
        return this._tempCameraPos;
    }

    _updateUniforms(cameraPos, sunVect) {
        this._uniforms.cPs.value.copy(cameraPos);
        this._uniforms.v3LightPosition.value.copy(sunVect);
    }

    updateStatic() {
        // For when the Earth will not rotate at all: possibly shown as ECEF plots where the Earth is fixed.
        const cameraPos = this._updateCameraPosition();
        this._updateUniforms(cameraPos, this.sunvect);
    }

    updateYAxisRotation() {
        // For when the Earth is rotating around its axis (with Y as up): the classic spinning Earth on its axis.
        const cameraPos = this._updateCameraPosition();
        const sunVect = this._tempSunVect.copy(this.sunvect);
        cameraPos.applyAxisAngle(this._axisY, -this.rotation.y);
        sunVect.applyAxisAngle(this._axisY, -this.rotation.y);
        this._updateUniforms(cameraPos, sunVect);
    }

    updateFullRotation() {
        // For when any rotations are happening: the Earth will be moving and rotating in any direction.
        const cameraPos = this._updateCameraPosition();
        const sunVect = this._tempSunVect.copy(this.sunvect);
        
        this._tempQuaternion
            .multiplyQuaternions(this.parentObj.quaternion, this.quaternion)
            .invert();
        
        cameraPos.applyQuaternion(this._tempQuaternion);
        sunVect.applyQuaternion(this._tempQuaternion);
        
        this._updateUniforms(cameraPos, sunVect);

        /*
        // Create container for orbital motion (separate from Earth's rotation)
        earth = new Earth3d(camera);

        const earthContainer = new Group();
        scene.add(earthContainer);
        // Tell Earth3d about its parent container
        earthContainer.add(earth);
        earth.parentObj = earthContainer;

        function animate() {
            // Earth spinning on multiple axes
            earth.rotation.y = time;
            earth.rotation.z = time/2;
            earth.rotation.x = -time/3;

            // Parent container also rotating and moving in orbit
            earthContainer.rotation.y = 2*time;
            earthContainer.rotation.z = 2*time/2;
            earthContainer.rotation.x = -2*time/3;

            earthContainer.position.x = 1.5 * Math.cos(time);
            earthContainer.position.z = 1.5 * Math.sin(time);

            sun.x = Math.cos(time);
            sun.y = Math.sin(time);
            earth.setSun(sun);

            earth.updateFullRotation();
        }
    ...
    setSunXYZ(x, y, z) {
        this.sunvect.set(x, y, z).normalize();
    }

    setSun(r) {
        this.sunvect.copy(r).normalize();
    }
}